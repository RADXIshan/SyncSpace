...
const JoinedOrgDash = ({ activities }) => {
  ...
  const getMeetingIcon = (title) => {
    ...
    if (
      ...
      return "🎨";
    if (lowerTitle.includes("planning") || lowerTitle.includes("sprint"))
      return "📋";
    if (lowerTitle.includes("review") || lowerTitle.includes("demo"))
      return "👀";
    if (lowerTitle.includes("retrospective") || lowerTitle.includes("retro"))
      return "🔄";
    if (lowerTitle.includes("sync") || lowerTitle.includes("meeting"))
      return "📞";
    return "🕓";
  };
  ...
  return (
    <>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-6 auto-rows-auto lg:auto-rows-[300px]">
          ...
          <div className="relative z-10 p-4 sm:p-6 lg:p-8 flex flex-col h-full">
            <div className="flex items-center gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-6 lg:mb-8">
              <div className="p-2 sm:p-3 lg:p-4 rounded-full bg-purple-500/20 border border-purple-500/30 group-hover/activity:bg-purple-500/30 transition-all duration-300 flex-shrink-0">
                <Activity
                  size={18}
                  className="text-purple-400 group-hover/activity:scale-110 transition-all duration-300 sm:w-6 sm:h-6"
                />
              </div>
            ...
            <div className="flex-1 overflow-hidden">
              <ul className="space-y-2 sm:space-y-3 max-h-full overflow-y-auto pr-1 sm:pr-2 scrollbar-thin scrollbar-thumb-purple-500/30 scrollbar-track-transparent">
                {activities.map((a, i) => (
                  <li
                    key={i}
                    className="group/card relative bg-gray-800/50 hover:bg-gray-700/50 border border-gray-600/50 hover:border-purple-500/50 rounded-lg sm:rounded-xl p-3 sm:p-4 cursor-pointer transition-all duration-300 transform hover:shadow-lg hover:shadow-purple-500/10"
                  >
                    ...
                    <div className="relative z-10 flex items-center gap-2 sm:gap-3">
                      ...
                      <span className="text-sm sm:text-base text-gray-300 group-hover/card:text-gray-200 transition-colors duration-300">
                        {a}
                      </span>
                    </div>
                  </li>
        ...
        <section className="order-3 lg:order-3 col-span-1 lg:col-span-6 relative row-span-1 bg-gray-900/95 backdrop-blur-xl border border-gray-700/50 rounded-2xl sm:rounded-3xl shadow-2xl group/meetings flex flex-col transition-all duration-500 mb-4 sm:mb-0 overflow-hidden px-6 py-4">
          {/* Background gradient overlay */}
          <div className="absolute inset-0 bg-gradient-to-br from-purple-900/20 via-gray-900/50 to-indigo-900/20"></div>

          <div className="relative flex flex-col gap-4 h-full">
            <div className="flex items-center gap-2">
              <div className="p-2.5 rounded-full bg-purple-500/20 border border-purple-500/30 group-hover/meetings:bg-purple-500/30 transition-all duration-300 flex-shrink-0">
                <Calendar
                  size={22}
                  className="text-purple-400 group-hover/meetings:scale-110 transition-all duration-300"
                />
              </div>
              <div className="text-xl font-bold text-white group-hover/meetings:text-purple-100 transition-colors duration-300">
                Upcoming Meetings
              </div>
            </div>
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between">
                          <p className="text-sm sm:text-base text-gray-300 group-hover/card:text-gray-200 transition-colors duration-300">
                            {getMeetingIcon(meeting.title)}{" "}
                            {meeting.displayTime} — {meeting.title}
                          </p>
                          <span
            ...
            <button
              ...
              className="px-4 sm:px-5 py-2 text-sm sm:text-base font-semibold rounded-full bg-purple-600/20 hover:bg-purple-600/30 border border-purple-500/30 text-purple-400 hover:text-purple-300 transition-all duration-300 transform hover:scale-105 active:scale-95 shadow-lg cursor-pointer"
            >
              View Calendar
            </button>
          </div>
