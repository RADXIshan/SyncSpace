...
const AIAssistant = ({ onClose, context }) => {
  ...
  const getInitialMessage = () => {
    // Meeting page context
    if (context?.page === 'meeting') {
      const participantsList = context.peers?.map(peer => 
        `  • ${peer.name || peer.email} ${!peer.videoEnabled ? '(camera off)' : ''} ${!peer.audioEnabled ? '(muted)' : ''}`
      ).join('\n') || '';
      
      const screenSharingStatus = context.localSettings?.isScreenSharing 
        ? `Active - ${context.localSettings.screenSharingUser} is sharing`
        : 'Inactive';
      
      const chatMessageCount = context.chatMessages?.length || 0;
      const chatInfo = chatMessageCount > 0 
        ? `\n💬 Meeting Chat:\n• ${chatMessageCount} message${chatMessageCount !== 1 ? 's' : ''} in chat\n• I have access to the full chat transcript`
        : '\n💬 Meeting Chat:\n• No messages yet\n• I can see messages as they come in';
      
      return {
        role: "assistant",
        content: `👋 Hi ${context.currentUser?.name || 'there'}! I'm your SyncSpace AI assistant with full meeting context.\n\n📹 Current Meeting Details:\n• Meeting ID: ${context.meetingId}\n• Total Participants: ${context.totalParticipants}\n\n👥 Participants:\n  • You (${context.currentUser?.name || context.currentUser?.email}) ${!context.localSettings?.videoEnabled ? '(camera off)' : ''} ${!context.localSettings?.audioEnabled ? '(muted)' : ''}\n${participantsList}\n\n🎥 Your Settings:\n• Video: ${context.localSettings?.videoEnabled ? '✅ On' : '❌ Off'}\n• Audio: ${context.localSettings?.audioEnabled ? '✅ On' : '🔇 Muted'}\n• Screen Sharing: ${screenSharingStatus}${chatInfo}\n\n🎯 I Can Help You With:\n• Summarizing chat discussions\n• Taking meeting notes and action items\n• Answering questions about the chat\n• Meeting controls and troubleshooting\n• Managing participants\n• Screen sharing tips\n• Audio/video troubleshooting\n\n💡 Plus All Regular Features:\n• Access to your workspace data\n• Organization and channel info\n• Meeting reports and analytics\n• Notes and documents\n• Calendar events\n\n✨ Try asking:\n• "Summarize the chat so far"\n• "What did [name] say about [topic]?"\n• "Take notes from this meeting"\n• "Who's in this meeting?"\n• "Help me with screen sharing"\n\nWhat would you like help with?`,
      };
    }
    
    // Channel page context
    if (context?.page === 'channel' && context?.channelId) {
      return {
        role: "assistant",
        content: `👋 Hi! I'm your SyncSpace AI assistant with full channel context.\n\n📢 Current Context:\n• You're viewing a channel\n• Channel ID: ${context.channelId}\n• I have access to this channel's messages, members, and activity\n\n🔍 I Have Full Access To:\n• This channel's messages and history\n• Channel members and their roles\n• Pinned messages and important discussions\n• Your organizations and all channels\n• Team members, roles, and permissions\n• Who's online right now\n• Scheduled meetings and active meetings\n• Meeting reports with summaries\n• Notes and documents\n• Notices and announcements\n• Calendar events\n\n✨ Try asking:\n• "Summarize recent messages in this channel"\n• "Who are the members of this channel?"\n• "What were the key discussions today?"\n• "Show me pinned messages"\n• "What's my organization name?"\n• "Who's online right now?"\n• "What meetings are scheduled?"\n\n💡 I can help with:\n• Channel-specific questions\n• Message summaries and insights\n• Team collaboration\n• Meeting scheduling\n• Document management\n• Best practices\n\nWhat would you like to know?`,
      };
    }
    
    // Dashboard/Messages/Calendar/etc context
    const pageNames = {
      'dashboard': 'Dashboard',
  ...
  const gatherContext = useCallback(async () => {
    try {
      ...
      const orgsResponse = await axios.get(
        `${baseURL}/api/orgs/user/organizations`,
      ...
      const currentOrg = currentOrgId
        ? organizations.find((org) => org.org_id === parseInt(currentOrgId))
        : null;
      ...
      if (currentOrgId) {
        try {
          ...
          console.log(
            "📡 Fetching organization details for org:",
            currentOrgId
          );
          const orgResponse = await axios.get(
            `${baseURL}/api/orgs/${currentOrgId}`,
            { withCredentials: true }
          );
          ...
          const membersResponse = await axios.get(
            `${baseURL}/api/orgs/${currentOrgId}/members`,
            { withCredentials: true }
          );
          ...
          const meetingsResponse = await axios.get(
            `${baseURL}/api/meetings?org_id=${currentOrgId}`,
            { withCredentials: true }
          );
          ...
          try {
            ...
            const reportsResponse = await axios.get(
              `${baseURL}/api/meeting-reports/organization/${currentOrgId}`,
              { withCredentials: true }
            );
          ...
          try {
            ...
            const notesResponse = await axios.get(
              `${baseURL}/api/notes?org_id=${currentOrgId}`,
              { withCredentials: true }
            );
          ...
          try {
            ...
            const noticesResponse = await axios.get(
              `${baseURL}/api/notices?org_id=${currentOrgId}`,
              { withCredentials: true }
            );
            ...
            console.error(
              ...
              noticesError.response?.status,
              noticesError.response?.data || noticesError.message
            );
            notices = [];
          }
          ...
          try {
            ...
            const eventsResponse = await axios.get(
              `${baseURL}/api/events?user_id=${user?.user_id}&org_id=${currentOrgId}`,
              { withCredentials: true }
            );
            ...
            console.error(
              ...
              eventsError.response?.status,
              eventsError.response?.data || eventsError.message
            );
            events = [];
          }
        } catch (error) {
        ...
        console.log(
          "⚠️ No current organization - user may not be in an org yet"
        );
      }
  ...
  useEffect(() => {
    ...
    const handleMeetingEnd = (data) => {
      ...
      setRealtimeContext((prev) => ({
        ...prev,
        activeMeetings: prev.activeMeetings.filter(
  ...
  // Update meeting context when it changes
  useEffect(() => {
    if (context?.page === 'meeting') {
      console.log('🔄 Updating meeting context in realtimeContext:', {
        chatMessages: context.chatMessages?.length || 0,
        participants: context.totalParticipants
      });
      setRealtimeContext((prev) => ({
        ...prev,
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const contextToSend = {
        ...
        meetingContext: realtimeContext.meetingContext || null,
      };

      console.log("Sending context to AI:", contextToSend);
      console.log("Meeting context in contextToSend:", contextToSend.meetingContext);
      if (contextToSend.meetingContext?.chatMessages) {
        console.log("Chat messages in context:", contextToSend.meetingContext.chatMessages);
      }
