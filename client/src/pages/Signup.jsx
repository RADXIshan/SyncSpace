      ...
      <div className="w-full md:w-[45%] flex items-center justify-center p-8">
        <form className="p-8 w-full max-w-md" onSubmit={handleSubmit}>
          ...
          <div className="mb-4">
            ...
            <input type="text" id="firstName" name="first_name" placeholder="Enter your first name" className={`w-full px-4 py-2 border rounded-lg focus:outline-none ${errors.first_name ? "border-secondary focus:border-red-400" : "focus:border-secondary"}`} value={formData.first_name} onChange={handleChange} />
          </div>
          <div className="mb-4">
            <label htmlFor="lastName" className="block mb-2 text-m font-medium text-gray-700">Last Name</label>
            <input type="text" id="lastName" name="last_name" placeholder="Enter your last name" className={`w-full px-4 py-2 border rounded-lg focus:outline-none ${errors.last_name ? "border-secondary focus:border-red-400" : "focus:border-secondary"}`} value={formData.last_name} onChange={handleChange} />
          </div>
          <div className="mb-4">
          ...
          <div className="mb-6 relative">
            ...
            <input type={showPassword ? "password" : "text"} id="password" name="password" placeholder="Enter your password" className={`w-full px-4 py-2 border rounded-lg focus:outline-none pr-10 ${errors.password ? "border-secondary focus:border-red-400" : "focus:border-secondary"}`} value={formData.password} onChange={handleChange} />
          ...
          <div className="mb-6">
            <button type="submit" className="w-full bg-secondary text-white py-2 rounded-lg font-semibold text-m hover:bg-purple-700 transition ease-in-out duration-300 hover:scale-105 active:scale-95 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" disabled={loading || Object.values(errors).some(Boolean)}>{loading ? "Signing up..." : "Signup"}</button>
          </div>
          <p className="mb-6 text-center text-gray-600">Already have an account? <Link to="/login" className="text-purple-600 hover:underline transition ease-in-out duration-300">Login</Link></p>
        </form>
      </div>
    </div>
  )
